...
# ----------------------------------------------------------------
# Configuration for CHNG outpatient insurance claims dataset count forecasting using DelphiRF
# ----------------------------------------------------------------
config <- list(

  # ---- Dataset metadata ----
  geo_level        = "state",           # Aggregation level
  indicator        = "chng_outpatient", # Source indicator name
  signal           = "covid_claims",    # Signal type
  ...
  start_date       = as.Date("2021-06-01"),  # Start date for experiments
  end_date         = as.Date("2023-01-10"),  # End date for experiments
  training_window  = 180,                    # Number of training days
  testing_window   = 30,                     # Interval between test windows
  ref_lag          = 60,                     # Maximum lag used for training targets
  lag_pad          = 1,                      # Padding before lag group models

  # ---- Preprocessing column mappings ----
  value_col        = "value_covid",           # Target value column in the raw data
  ...
  smoothed_target  = FALSE,         # Whether to use smoothed as the target
